...
const Checkout = () => {
    ...
    const { data, isLoading: getPackageIdLoading } = useGetPackageByIdQuery(packageId);
    const [createBooking, { isLoading, isError, isSuccess, error }] = useCreateBookingMutation();
        ...
        <>
            <div className='md:w-1/3 w-full mx-auto my-20'>
                ...
                <form onSubmit={handleSubmit} className='flex flex-col leading-10 max-w-full px-2'>
                    ...
                    <input type="email" name="email" id="email" disabled placeholder='Email' value={email} className='px-3 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-fuchsia-400 mb-3' required />

                    <input type="number" name="phone" id="phone" placeholder='Phome Number' required
                        className='px-3 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-fuchsia-400 mb-3'
                    />

                    <textarea rows='3' cols='2' placeholder='Address Here...' name="address" className='px-3 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-fuchsia-400 ' />
